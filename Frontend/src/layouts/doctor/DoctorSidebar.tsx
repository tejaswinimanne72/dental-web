...
const navItems = [
  { to: "/app/DoctorDashboard",    label: "Overview",       Icon: LayoutDashboardIcon },
  { to: "/doctor/appointments",    label: "Schedule",       Icon: CalendarDaysIcon    },
  ...
  { to: "/doctor/help",            label: "Help",           Icon: HelpCircleIcon      },
];

export const DoctorSidebar: React.FC = () => {
  const navigate = useNavigate();
  const userName = localStorage.getItem("userName") || "Doctor";
  const initials = userName.split(" ").map((w) => w[0]).join("").slice(0, 2).toUpperCase() || "DR";
  ...
  /* Nav link class — emerald accent for doctor */
  const navLinkClass = ({ isActive }: { isActive: boolean }) =>
    [
      "flex items-center gap-2 px-3 py-2 text-[13px] font-medium rounded-xl transition-all duration-150 whitespace-nowrap",
      isActive
        ? "bg-emerald-500/18 text-emerald-300 border border-emerald-500/25 shadow-[0_0_0_1px_rgba(16,185,129,0.15),0_2px_10px_-4px_rgba(16,185,129,0.25)]"
        ...
        <div className="flex h-16 items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <div className="flex flex-col leading-none">
              <span className="text-[14px] font-bold tracking-tight text-slate-50">Doctor Console</span>
              <span className="text-[10px] text-slate-500 mt-0.5">Dental Clinic AI</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <div className="hidden sm:flex items-center gap-2 px-2.5 py-1.5 rounded-xl bg-slate-900 border border-slate-800">
              <div className="w-6 h-6 rounded-lg bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center text-white font-bold text-[10px] shrink-0">
                {initials}
              </div>
              <span className="text-[12px] font-semibold text-slate-200 max-w-[100px] truncate">{userName}</span>
            </div>
